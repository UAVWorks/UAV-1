...
CGColorRef graphBackgroundColor()
{
	...
	if(c == NULL)
	{
		...
		c = CreateDeviceRGBColor(0, 0, 0, 1.0);
...
{
	for(CGFloat y = 8.5; y <= 105.5; y += 16.0)
	{
...
@interface GraphViewSegment : NSObject
{
	...
	NSInteger xhistory[33];
	NSInteger yhistory[33];
	NSInteger zhistory[33];
	int index;
}

// returns true if adding this value fills the segment, which is necessary for properly updating the segments
-(BOOL)addX:(NSInteger)x y:(NSInteger)y z:(NSInteger)z; 
...
-(id)init
{
	...
	if(self != nil)
	{
		...
		layer.bounds = CGRectMake(0.0, 0.0, 32.0, 112.0);
...
//	if(xhistory[0] != 0){
	...
	if(xhistory[0] == 0){
		...
		[[GraphTextView alloc] initWithFrame:CGRectMake(0.0, 0.0, 32.0, 112.0)];
	}	
	for(i = 0; i < 32; ++i)
	{
		lines[i*2].x = 32-i;
		lines[i*2].y = 104 - xhistory[i] * 1.6;
		lines[i*2+1].x = 32-(i + 1);
		lines[i*2+1].y = 104 - xhistory[i+1] * 1.6;
	}
...
//	if(yhistory[0] != 0){
	...
	{ 
		...
		//uses the .x from X above since same time frame.
		lines[i*2].y = 104 - yhistory[i] * 1.6;
		lines[i*2+1].y = 104 - yhistory[i+1] * 1.6;
	}
...
//	if(zhistory[0] != 0){
	...
	{
		//uses the .x from X above since same time frame.
		lines[i*2].y = 104 - zhistory[i] * 1.6;
		lines[i*2+1].y = 104 - zhistory[i+1] * 1.6;
	}
...
-(void)drawRect:(CGRect)rect
{
	...
	[@"  60" drawInRect:CGRectMake(2.0, 0.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
//	[@"  50" drawInRect:CGRectMake(2.0, 16.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"  40" drawInRect:CGRectMake(2.0, 32.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
//	[@"  30" drawInRect:CGRectMake(2.0, 48.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"  20" drawInRect:CGRectMake(2.0, 64.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
//	[@"  10" drawInRect:CGRectMake(2.0, 80.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"   0" drawInRect:CGRectMake(2.0, 96.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	//UIFont *systemFont = [UIFont systemFontOfSize:12.0];
}
...
-(void)addX:(NSInteger)x y:(NSInteger)y z:(NSInteger)z
{
...
#define kSegmentInitialPosition CGPointMake(640.0, 56.0); //the starting point of drawing lines. 
